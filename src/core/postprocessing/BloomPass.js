...
export class BloomPass extends Pass {
  ...
  _createDefaultDirtTexture() {
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        const val = Math.max(
          0,
          noise * 0.5 + 0.5 - vignette * 0.3 + dust * 0.5,
        );
  ...
  _createRenderTargets(width, height, mipW, mipH) {
    ...
    this._renderTargetBright = new THREE.WebGLRenderTarget(
      width,
      height,
      rtOptions,
    );
    ...
    this._renderTargetBlurH1 = new THREE.WebGLRenderTarget(
      mipW,
      mipH,
      rtOptions,
    );
    ...
    this._renderTargetBlurV1 = new THREE.WebGLRenderTarget(
      mipW,
      mipH,
      rtOptions,
    );
    ...
    this._renderTargetBlurH2 = new THREE.WebGLRenderTarget(
      mipW,
      mipH,
      rtOptions,
    );
    ...
    this._renderTargetBlurV2 = new THREE.WebGLRenderTarget(
      mipW,
      mipH,
      rtOptions,
    );
    ...
    this._renderTargetBlurH3 = new THREE.WebGLRenderTarget(
      mipW,
      mipH,
      rtOptions,
    );
    ...
    this._renderTargetBlurV3 = new THREE.WebGLRenderTarget(
      mipW,
      mipH,
      rtOptions,
    );
    ...
    this._renderTargetComposite = new THREE.WebGLRenderTarget(
      width,
      height,
      rtOptions,
    );
  ...
  render(renderer, writeBuffer, readBuffer, deltaTime, maskActive) {
    ...
    if (this._qualitySettings.mipLevel > 0) {
      ...
      this._blurHMaterial.uniforms.mipLevel.value =
        this._qualitySettings.mipLevel;
    ...
    for (let pass = 0; pass < blurPasses; pass++) {
      ...
      this._blurHMaterial.uniforms.kernelSize.value =
        this._qualitySettings.kernelSize;
      ...
      this._blurVMaterial.uniforms.kernelSize.value =
        this._qualitySettings.kernelSize;
      this._blurVMaterial.uniforms.sigma.value = this.radius * 2.0;
      this._blurVMaterial.uniforms.mipLevel.value =
        this._qualitySettings.mipLevel;
  ...
  _updateUniforms() {
    ...
    this._compositeMaterial.uniforms.colorWeight.value.copy(
      this.customColorWeight,
    );
  }
  ...
  setParams(params) {
    ...
    if (params.dirtIntensity !== undefined)
      this.dirtIntensity = params.dirtIntensity;
    if (params.colorWeight !== undefined)
      this.customColorWeight.copy(params.colorWeight);
  }
